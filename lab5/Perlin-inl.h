...
public:
  ...
  PerlinNoise() {
    ...
    gradientVec.insert(gradientVec.end(), gradientVec.begin(),
                       gradientVec.end());
  }
  ...
  double noise(double x, double y, double z) {
    ...
    int A = gradientVec[X] + Y;
    int AA = gradientVec[A] + Z;
    int AB = gradientVec[A + 1] + Z;
    int B = gradientVec[X + 1] + Y;
    int BA = gradientVec[B] + Z;
    int BB = gradientVec[B + 1] + Z;

    // Add blended results from 8 corners of cube
    double res =
        lerp(w,
             lerp(v,
                  lerp(u, grad(gradientVec[AA], x, y, z),
                       grad(gradientVec[BA], x - 1, y, z)),
                  lerp(u, grad(gradientVec[AB], x, y - 1, z),
                       grad(gradientVec[BB], x - 1, y - 1, z))),
             lerp(v,
                  lerp(u, grad(gradientVec[AA + 1], x, y, z - 1),
                       grad(gradientVec[BA + 1], x - 1, y, z - 1)),
                  lerp(u, grad(gradientVec[AB + 1], x, y - 1, z - 1),
                       grad(gradientVec[BB + 1], x - 1, y - 1, z - 1))));
    return (res + 1.0) / 2.0;
  }
